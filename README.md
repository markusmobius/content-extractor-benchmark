...
The builder archives the requested Trafilatura tags and uses each tag's locked dependency graph; it adds only the common benchmark adapter. Omit `--candidate-ref` to snapshot all three candidate working trees, including local changes. `--offline` requires already cached Cargo dependencies. The measured candidate was a recorded worktree snapshot; release packaging must preserve those runtime file hashes. The new APIs share an immutable input, not a single internal mutable DOM representation. The production `rustHTML` worker remains unchanged.
